...
public class Villager : Characters, IWorker
{
    public override void Killed()
    {
        base.Killed();
        Debug.Log("i am a Villager");
    ...
    void Update()
    {
        if(state == State.InQueue)
        {
            if(!IsArrived())//in queue
            {
                ...
                //transform.position = Vector3.Lerp(transform.position, currentTransform.position, 5 * Time.deltaTime);
